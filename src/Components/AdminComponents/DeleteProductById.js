...
function DeleteProductById() {
    ...
    useEffect(() => {
        const checkIsAdmin = async () => {
          ...
          try {
            const response = await FetchWithAuth('http://proyecto-final-back-end-a466e2d08fab.herokuapp.com/product/profile-info', {
                method: 'GET',
                headers: {
    ...
    const handleDelete = async () => {
        try {
            ...
            const response = await FetchWithAuth(`http://proyecto-final-back-end-a466e2d08fab.herokuapp.com/product/${productId}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${accessToken}`
